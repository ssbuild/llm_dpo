...
if global_args["enable_lora"]:
    from config.dpo_config_lora import train_info_args,train_info_args_hf,train_info_args_colossalai,train_model_config
elif global_args["enable_ptv2"]:
    raise NotImplemented
else:
    from config.dpo_config import train_info_args,train_info_args_hf,train_info_args_colossalai,train_model_config
...
def patch_args(train_info_args):
    ...
    # 预处理
    if 'rwkv' in (train_info_args.get('model_name_or_path', '') or train_info_args.get('tokenizer_name', '')).lower():
